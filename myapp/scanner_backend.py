...
def get_extended_hardware_specs():
    ...
    if platform.system() == "Windows":
                    ...
                    specs["gpu"].append(
                        {
                            ...
                            "vram_gb": round(g.get("AdapterRAM", 0) / (1024**3), 2)
                            if g.get("AdapterRAM")
                            else "Unknown",
                        }
                    )
        ...
        try:
            bios_data = run_powershell_cmd(
                "Get-CimInstance Win32_BIOS | Select-Object Manufacturer, Name, Version | ConvertTo-Json"
            )
            if bios_data:
