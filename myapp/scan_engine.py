...
COMMON_PORTS = {
    20: (
        "FTP data",
        "File transfer channel. Risky when exposed without encryption.",
        "high",
    ),
    21: ("FTP", "Plain FTP can expose credentials and files.", "high"),
    22: (
        "SSH",
        "Remote administration. Safe when patched and key protected.",
        "medium",
    ),
    ...
    53: (
        "DNS",
        "Name resolution. Public exposure can enable abuse if recursive.",
        "medium",
    ),
    ...
    445: (
        "SMB",
        "Windows file sharing. Harmful when exposed to the internet.",
        "critical",
    ),
    ...
    8000: (
        "Django dev server",
        "Development web server. Do not expose publicly.",
        "medium",
    ),
...
def _file_entropy_hint(path):
    ...
    entropy = -sum(
        (count / len(sample)) * log2(count / len(sample)) for count in counts.values()
    )
...
def _file_soc_details(path, risk, malicious, sha256):
    return {
        "reputation": "Unavailable: no reputation conclusion from filename or extension",
        "md5": "Not computed",
        "sha1": "Not computed",
        "sha256": sha256,
        "real_type": "Extension hint: " + path.suffix.lower(),
        "extension_mismatch": path.name.lower().endswith(
            (".jpg.exe", ".pdf.exe", ".pdf.scr")
        ),
        "entropy": _file_entropy_hint(path),
        "signature": "Not verified",
        "location_analysis": str(path.parent),
        "yara": "Not run in host snapshot; use uploaded-file analysis for optional YARA",
        "pe_analysis": "Not performed in host snapshot",
        "persistence": "Not inferred from file presence",
        "behavior": "Static observation only; execution not observed",
        "mitre": [],
    }
...
def profile_system():
    ...
    return {
        ...
        "mac_address": ":".join(
            f"{(uuid.getnode() >> bits) & 0xFF:02x}" for bits in range(40, -1, -8)
        ),
...
def process_audit(limit=80):
    ...
    for proc in psutil.process_iter(
        ["pid", "name", "exe", "username", "cpu_percent", "memory_percent", "cmdline"]
    ):
        try:
            info = proc.info
            ...
            if "powershell" in name and (
                "-enc" in cmdline.lower() or "frombase64string" in cmdline.lower()
            ):
                suspicious.append(
                    {
                        "pid": info.get("pid"),
...
def _contains_credential(text):
    ...
    return bool(
        text and any(pattern.search(text) for pattern in CREDENTIAL_PATTERNS.values())
    )
            ...
            entries.append(
                {
                    ...
                    "reason": "Script or executable starts automatically."
                    if risky
                    else "Normal startup item.",
                    "resolution": "Disable unknown startup entries and verify publisher."
                    if risky
                    else "No action needed if recognized.",
                }
            )
...
def active_network_connections(limit=120):
    ...
    for conn in psutil.net_connections(kind="inet"):
        ...
        service, why, risk = COMMON_PORTS.get(
            port, ("Unknown service", "Open by an application or OS service.", "medium")
        )
    ...
    for port in ports:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            if sock.connect_ex((host, port)) == 0:
                service, why, risk = COMMON_PORTS.get(
                    port, ("Unknown service", "Open by an application.", "medium")
                )
...
def file_scan(max_files=300):
    ...
    for root in roots:
        ...
        for path in root.rglob("*"):
            ...
            suspicious = suffix in SUSPICIOUS_EXTENSIONS or any(
                token in name for token in SUSPICIOUS_NAMES
            )

            credential_matches = (
                credential_file_scan(path)
                if suffix in TEXT_EXTENSIONS or path.name.lower().startswith(".env")
                else []
            )
            ...
            if suspicious:
                risk = (
                    "high"
                    if suffix in {".exe", ".dll", ".scr", ".ps1", ".vbs"}
                    else "medium"
                )
                ...
                findings.append(
                    {
                        ...
                        "reason": _file_reason(
                            path, "critical", "Credential Exposure", match
                        ),
                        "resolution": _file_resolution(
                            path, "critical", "Credential Exposure"
                        ),
                        **_file_soc_details(
                            path, "critical", "Credential Exposure", sha256
                        ),
                    }
                )
            ...
            shares.append(
                {
                    ...
                    "risk": "medium",
                    "reason": "Removable storage can introduce untrusted files.",
                }
            )
...
def identity_privilege_assessment(user=None):
    ...
    return {
        ...
        "reason": "Superuser accounts should be used only for administration."
        if getattr(user, "is_superuser", False)
        else "Standard user privileges reduce blast radius.",
    }
...
def threat_database_matches(files, ports):
    from .analysis.ioc_correlation import correlation_payload
    matches = correlation_payload({"hashes": [x.get("sha256", "") for x in files["files"]]})["matches"]
    return [{"match": "Exact SHA-256 match in operator-maintained local IOC dataset", "confidence": "Exact value match; source validity requires review", "ioc": x["value"]} for x in matches if x["ioc_type"]=="HASH"]


def threat_intel_lookup():

    # Reports honestly whether external threat-intel providers are actually

    # configured (via VT_API_KEY / ABUSEIPDB_API_KEY) instead of always

    # claiming "offline" or fabricating a connection. See settings.py.

    try:
        from django.conf import settings

        vt_configured = bool(getattr(settings, "VT_API_KEY", ""))

        abuseipdb_configured = bool(getattr(settings, "ABUSEIPDB_API_KEY", ""))
    ...
    if vt_configured:
        providers.append(
            "VirusTotal (key configured, live lookups not wired in this build)"
        )

    if abuseipdb_configured:
        providers.append(
            "AbuseIPDB (key configured, live lookups not wired in this build)"
        )
    ...
    if not providers:
        return {
            ...
            "sources": sources,
            "note": "No external provider API keys are configured (VT_API_KEY, ABUSEIPDB_API_KEY). This build uses local analysis only.",
        }

    return {
        "status": "PARTIAL - keys configured, live queries not implemented",
        "sources": sources + providers,
        "note": "External provider keys are present in the environment, but this build does not yet perform live API calls. Treat provider fields as THREAT INTELLIGENCE UNAVAILABLE until that integration is built and tested.",
    }
...
def ai_recommendations(score, ports, files, processes, startup):
    ...
    if score < 70:
        recommendations.append(
            "Priority: reduce exposed services, review suspicious files, and patch endpoint protection before normal use."
        )

    if any(item["risk"] in {"high", "critical"} for item in ports):
        recommendations.append(
            "High-risk ports are open. Restrict remote administration and file-sharing ports to trusted networks only."
        )

    if files["detected_count"]:
        recommendations.append(
            "Suspicious files were found. Quarantine unknown files, verify hashes, and avoid running downloaded scripts."
        )

    if processes:
        recommendations.append(
            "Suspicious processes are running. Check the executable path, digital signature, and parent process."
        )

    if any(item.get("malicious") == "Credential Exposure" for item in files["files"]):
        recommendations.append(
            "Credential material was detected. Rotate exposed secrets immediately and move credentials to a secure vault."
        )

    if startup:
        recommendations.append(
            "Review startup entries so unknown scripts do not run automatically after reboot."
        )

    if not recommendations:
        recommendations.append(
            "Risk is currently low. Keep updates enabled, retain firewall rules, and scan downloads before opening them."
        )

    recommendations.append(
        "For deeper analysis, review the linked blogs about open ports, malware triage, MITRE mapping, and SOC response."
    )
...
def _scan_scope_payload(
    scan_type, connections, ports, files, processes, startup, behavioral, threat_db
):

    normalized_type = (scan_type or "combined").lower()

    empty_files = {
        "scanned_count": files["scanned_count"],
        "detected_count": 0,
        "files": [],
    }
    ...
    return {
        ...
        "modules": {
            ...
            "real_time_monitoring": {
                "status": "Snapshot only; continuous monitoring unsupported",
                "monitored": ["Downloads", "Desktop", "Documents", "WhatsApp"],
            },
            ...
            "mitre_attack": [],
            "quarantine": {"available": False, "mode": "Unsupported"},
            "remediation": {
                "actions": [
                    "kill process",
                    "delete file",
            ...
            "incident_response": {"create_incident_on_critical": False},
            "threat_timeline": [
                {"event": "Collection completed", "risk": "info"},
                {"event": "Analysis completed", "risk": risk},
                {"event": "Report generated", "risk": "info"},
            ],
            "edr": {"status": "Unsupported", "detail": "No EDR agent is connected."},
            "ndr": {
                "status": "Unsupported",
                "detail": "No packet or flow telemetry collected.",
            },
            "file_threat_analysis": {
                "status": "Heuristic Analysis",
                "detected": scoped_files["detected_count"],
            },
            "uba": {
                "status": "Unsupported",
                "detail": "No behavioral baseline is available.",
            },
            "risk_engine": {
                "formula": "Capped sum of configured severity weights over collected observations; heuristic, not a probability",
                "threat_score": score,
                "risk_level": risk,
            ...
            "soar": {
                ...
                "status": "Unsupported: no automated response executor.",
            },
            "executive_metrics": {
        ...
        "recommendations": ai_recommendations(
            score, scoped_ports, scoped_files, scoped_processes, scoped_startup
        ),
    }
